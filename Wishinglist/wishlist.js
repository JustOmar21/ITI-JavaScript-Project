...
function addCart(e)
{
   ...
   if (!shoppingCart.some((cart) => { return cart.productID == wish.productID && cart.userID == sessionStorage.getItem("currentUserID") })) {
   let cartID = localStorage.getItem("cartID") || 0;
   localStorage.setItem("cartID",Number(cartID)+1);
   let item = new ShoppingCart(cartID , wish.productID , 1 , sessionStorage.getItem("currentUserID"));
