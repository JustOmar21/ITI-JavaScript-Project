...
function getData() {
   ...
   let singleUserShip = userShips.filter((user) => user.userID == currentUserID)[0];
...
function submitFun(e) {
   ...
   let stateValidation = streetState && cityState && stateState && zipcodeState && countryState && phoneState;

   if (stateValidation) {
      let userShips = JSON.parse(localStorage.getItem("shippingInfo")) || [];
      let singleUser = userShips.filter((ship) => ship.userID == sessionStorage.getItem("currentUserID"))[0];
      if (singleUser) {
         ...
         let singleUserShip = new ShippingInfo(street.value, city.value, state.value, zipcode.value, country.value, phone.value, sessionStorage.getItem("currentUserID"));
      ...
      for (let prod of products) {
         if (order.productID == prod.id) {
            ...
            if (Number(order.quantity) > Number(prod.quantity)) {
               alert(`the "${prod.name}" product has only ${prod.quantity} units, please lower your order quantity`);
               location.assign("../shoppingCart");
            }
         }
      }
