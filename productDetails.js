document.addEventListener("DOMContentLoaded", function () {
   ...
   if (hamburgerMenu) { // Check if the element is found
...
if (localStorage.getItem("currentProductID")) {
   ...
   alert("You cannot access this page directly, please enter from the Category Page");
...
function generateProduct() {
   ...
   document.getElementById("product-price").textContent = `Price: ${currentProduct.price}$`;
...
function addToCartHandle(currentProduct) {
   ...
   if (!shoppingCartArr.some((product) => { return product.productID == currentProduct.id && product.userID == sessionStorage.getItem("currentUserID") })) {
      let cartID = localStorage.getItem("cartID") || 0;
      localStorage.setItem("cartID",Number(cartID)+1);
      let product = new ShoppingCart(cartID,currentProduct.id,1,sessionStorage.getItem("currentUserID"));
...
function addToWishlistHandle(currentProduct) {
   ...
   if (!Wishlists.some((wishlist) => { return wishlist.productID == currentProduct.id && wishlist.userID == sessionStorage.getItem("currentUserID") })) {
      let wishlistID = localStorage.getItem("wishlistID") || 0;
      localStorage.setItem("wishlistID",Number(wishlistID)+1);
      let product = new Wishlist(wishlistID,currentProduct.id,sessionStorage.getItem("currentUserID"));
