...
{
  let catID = e.target.parentElement.previousElementSibling.previousElementSibling.previousElementSibling.innerText;
...
function removeProduct(singleCat) 
{
  ...
  for (let singleProduct in products) {
    if (singleCat.id == products[singleProduct].categoryID) {
      removeOrder(products[singleProduct]);
...
function removeOrder(singleProduct) 
{
  let orders = localStorage.getItem("order");
  orders = orders ? JSON.parse(orders) : [];
  for (let singleOrder in orders) {
    if (singleProduct.id == orders[singleOrder].productID) {
      orders.splice(singleOrder, 1);
    }
  }
